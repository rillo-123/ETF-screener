...
# Build the python command string (calling from consolidated scripts folder)
$cmd = "set PYTHONPATH=src; python src/ETF_screener/scripts/churn_strategies.py --entry `"$Entry`" --exit `"$Exit`""
if ($Filter) {
    $cmd += " --filter `"$Filter`""
}
