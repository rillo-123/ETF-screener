...
def clean_price_data(series: pd.Series, max_pct_change: float = 0.5) -> pd.Series:
    ...
    for i in range(1, len(s)):
        ...
        # Disqualify if jump exceeds configured threshold or diverges strongly from median.
        if pct_change > max_pct_change or median_div > 0.80:
            s.iloc[i] = prev_val
...
def calculate_rsi_ema(
    data: Any, rsi_period: int = 14, ema_period: int = 10
) -> pd.Series:
    """
...
def calculate_adx(
    high: Any, low: Any = None, close: Any = None, period: int = 14
) -> pd.Series:
    """
    ...
    up_move = h - h.shift()
    down_move = lo.shift() - lo

    plus_dm = pd.Series(0.0, index=h.index)
    plus_dm[(up_move > down_move) & (up_move > 0)] = up_move

    minus_dm = pd.Series(0.0, index=lo.index)
...
def calculate_stochastic(
    high: Any, low: Any = None, close: Any = None, k_period: int = 14, d_period: int = 3
    ...
    low_min = lo.rolling(window=k_period).min()
...
def calculate_tsi(
    data: Any, long: int = 25, short: int = 13, signal: int = 13
) -> Tuple[pd.Series, pd.Series]:
    """
...
def calculate_supertrend(
    ...
    if start_idx >= len(c):
        return (
            supertrend
            if not isinstance(high, pd.DataFrame)
            else (supertrend, final_ub, final_lb)
        )

    # Initialize first valid bar
    # Use -1 (down) as initial guess if price is below high-low average
    supertrend_direction.iloc[start_idx] = (
        1 if c.iloc[start_idx] > hl2.iloc[start_idx] else -1
    )
    supertrend.iloc[start_idx] = (
        final_lb.iloc[start_idx]
        if supertrend_direction.iloc[start_idx] == 1
        else final_ub.iloc[start_idx]
    )
...
def add_indicators(
    ...
    st_res = calculate_supertrend(df_copy, period=st_period, multiplier=st_multiplier)
    ...
    df_copy["Pullback_Pct"] = (
        (df_copy["Recent_High"] - df_copy["Close"]) / df_copy["Recent_High"] * 100
    ).round(2)

    # Distance from EMA50 (negative = below, positive = above)
    df_copy["EMA_Distance_Pct"] = (
        (df_copy["Close"] - df_copy["EMA_50"]) / df_copy["EMA_50"] * 100
    ).round(2)
