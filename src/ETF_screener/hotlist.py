...
def generate_hotlist(
    ...
    try:
        ...
        print("[HOTLIST] Generating swing trading prospects...")
        ...
        if volume_results.empty:
            ...
            with open(hotlist_file, "w", encoding="utf-8") as f:
                f.write("=" * 80 + "\n")
                f.write(
                    f"SWING TRADING HOTLIST - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                )
                f.write("=" * 80 + "\n\n")
        ...
        with open(hotlist_file, "w", encoding="utf-8") as f:
            ...
            f.write("Criteria:\n")
            f.write("  • Green Supertrend (price > ST)\n")
            ...
            if swing_results.empty:
                ...
                print("[HOTLIST] No opportunities found (0 prospects)")
            else:
                ...
                if "Pullback%" in swing_results.columns:
                    swing_results = swing_results.sort_values(
                ...
                for idx, (_, row) in enumerate(swing_results.iterrows(), 1):
                    ...
                    if volume != "N/A":
                        try:
                            f.write(
                                f"   Avg Volume:    {int(volume):,}\n"
                                if isinstance(volume, (int, float))
                                else f"   Avg Volume:    {volume}\n"
                            )
                        except (ValueError, TypeError):
                ...
                f.write("\n" + "-" * 80 + "\n")
                print(
                    f"[HOTLIST] Found {len(swing_results)} swing trading opportunities"
                )
