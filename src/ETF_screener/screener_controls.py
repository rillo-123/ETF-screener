...
TIMELINE_RULE_KEYS = [*DEFAULT_TIMELINE_ORDER, "volume_spike", "price_ema", "ema_flatten", "ha_ema_volume"]
...
DEFAULT_SCREEN_FILTERS: dict[str, object] = {
    ...
    "ha_ema_volume_conditions": [dict(item) for item in DEFAULT_HA_EMA_VOLUME_CONDITIONS],
...
class RsiEvent:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, object]) -> "RsiEvent":
        ...
        return cls(
            ...
            event_id=(str(payload["rsi_event_id"]) if payload.get("rsi_event_id") else None),
        )
...
class RsiEventSequence:
    ...
    def find_ages(self, rsi: pd.Series, dates: pd.Series, *, lookback_days: int) -> list[int] | None:
        ...
        for event in self.events:
            mask = _rsi_cross_mask(rsi, level=event.level, mode=event.mode).fillna(False)
            event_dates = pd.to_datetime(dates[mask & (dates >= cutoff)], errors="coerce")
...
def _normalize_ha_ema_volume_conditions(raw: dict[str, object]) -> list[dict[str, object]]:
...
def _validate_ha_ema_volume_conditions(conditions: list[dict[str, object]]) -> list[str]:
    """Return contradictions implied by OHLC ordering and selected zones."""
    ranks = {"below_ema2": 0, "between_ema1_ema2": 1, "above_ema1": 2}
    active = {item["field"]: ranks[item["zone"]] for item in conditions if item.get("enabled")}
    contradictions = []
    ordered_pairs = [("low", "open"), ("low", "close"), ("low", "high"), ("open", "high"), ("close", "high")]
    for lower_field, upper_field in ordered_pairs:
        if lower_field in active and upper_field in active and active[lower_field] > active[upper_field]:
            contradictions.append(f"{lower_field.title()} cannot be in a higher zone than {upper_field.title()}.")
...
def normalize_screen_filters(payload: object | None = None) -> dict[str, object]:
    ...
    if not isinstance(raw_rsi_events, list) or not raw_rsi_events:
        raw_rsi_events = [raw]
    normalized_rsi_events: list[dict[str, object]] = []
    ...
    if isinstance(raw_price_ema_sources, list):
        selected_price_ema_sources = [str(source).strip().lower() for source in raw_price_ema_sources]
    else:
        selected_price_ema_sources = [str(raw.get("price_ema_source") or DEFAULT_PRICE_EMA_SOURCE).strip().lower()]
    selected_price_ema_sources = list(dict.fromkeys(source for source in selected_price_ema_sources if source in {"open", "high", "low", "close"})) or [DEFAULT_PRICE_EMA_SOURCE]
    raw_price_ema_events = raw.get("price_ema_events")
    if not isinstance(raw_price_ema_events, list) or not raw_price_ema_events:
        raw_price_ema_events = [{**raw, "price_ema_source": source} for source in selected_price_ema_sources]
    normalized_price_ema_events: list[dict[str, object]] = []
    for index, event in enumerate(raw_price_ema_events[:3]):
        payload = event if isinstance(event, dict) else {}
        normalized_price_ema_events.append({
            "price_ema_source": str(payload.get("price_ema_source") or DEFAULT_PRICE_EMA_SOURCE).strip().lower(),
            "price_ema_period": _coerce_int(payload.get("price_ema_period"), DEFAULT_PRICE_EMA_PERIOD, minimum=2, maximum=500),
            "price_ema_cross_mode": str(payload.get("price_ema_cross_mode") or DEFAULT_PRICE_EMA_CROSS_MODE).strip().lower(),
            "price_ema_event_age": _normalize_event_age(payload.get("price_ema_event_age"), lookback_days=lookback_days, default=DEFAULT_PRICE_EMA_EVENT_AGE),
            "price_ema_event_id": str(payload.get("price_ema_event_id") or payload.get("id") or ("price_ema" if index == 0 else f"price_ema_{index + 1}")),
        })
    ...
    filters = {
        ...
        "ema_relationship_fast_slope": str(raw.get("ema_relationship_fast_slope") or raw.get("ema_relationship_slope") or DEFAULT_EMA_RELATIONSHIP_SLOPE).strip().lower(),
        "ema_relationship_slow_slope": str(raw.get("ema_relationship_slow_slope") or raw.get("ema_relationship_slope") or DEFAULT_EMA_RELATIONSHIP_SLOPE).strip().lower(),
        "ema_relationship_cross_mode": str(raw.get("ema_relationship_cross_mode") or DEFAULT_EMA_RELATIONSHIP_CROSS_MODE).strip().lower(),
        ...
        "price_ema_event_enabled": _coerce_bool(raw.get("price_ema_event_enabled"), False),
        "price_ema_period": _coerce_int(raw.get("price_ema_period"), DEFAULT_PRICE_EMA_PERIOD, minimum=2, maximum=500),
        "price_ema_source": str(raw.get("price_ema_source") or DEFAULT_PRICE_EMA_SOURCE).strip().lower(),
        "price_ema_sources": selected_price_ema_sources,
        "price_ema_cross_mode": str(raw.get("price_ema_cross_mode") or DEFAULT_PRICE_EMA_CROSS_MODE).strip().lower(),
        "price_ema_event_age": _normalize_event_age(raw.get("price_ema_event_age"), lookback_days=lookback_days, default=DEFAULT_PRICE_EMA_EVENT_AGE),
        "price_ema_events": normalized_price_ema_events,
        "ema_flatten_enabled": _coerce_bool(raw.get("ema_flatten_enabled"), False),
        "ema_flatten_period": _coerce_int(raw.get("ema_flatten_period"), DEFAULT_EMA_FLATTEN_PERIOD, minimum=2, maximum=500),
        "ema_flatten_lookback": _coerce_int(raw.get("ema_flatten_lookback"), DEFAULT_EMA_FLATTEN_LOOKBACK, minimum=1, maximum=30),
        "ema_flatten_tolerance": _coerce_float(raw.get("ema_flatten_tolerance"), DEFAULT_EMA_FLATTEN_TOLERANCE, minimum=0.0, maximum=5.0),
        "ema_flatten_mode": str(raw.get("ema_flatten_mode") or DEFAULT_EMA_FLATTEN_MODE).strip().lower(),
        "ema_flatten_event_age": _normalize_event_age(raw.get("ema_flatten_event_age"), lookback_days=lookback_days, default=DEFAULT_EMA_FLATTEN_EVENT_AGE),
        "volume_spike_enabled": _coerce_bool(raw.get("volume_spike_enabled"), False),
        "volume_spike_period": _coerce_int(raw.get("volume_spike_period"), DEFAULT_VOLUME_SPIKE_PERIOD, minimum=2, maximum=100),
        "volume_spike_multiplier": _coerce_float(raw.get("volume_spike_multiplier"), DEFAULT_VOLUME_SPIKE_MULTIPLIER, minimum=1.1, maximum=10.0),
        "volume_spike_age": _normalize_event_age(raw.get("volume_spike_age"), lookback_days=lookback_days, default=5),
        "ha_ema_volume_enabled": _coerce_bool(raw.get("ha_ema_volume_enabled"), False),
        "ha_ema_volume_ema1_period": _coerce_int(raw.get("ha_ema_volume_ema1_period"), DEFAULT_HA_EMA_VOLUME_EMA1_PERIOD, minimum=2, maximum=500),
        "ha_ema_volume_ema1_source": str(raw.get("ha_ema_volume_ema1_source") or DEFAULT_HA_EMA_VOLUME_EMA1_SOURCE).strip().lower(),
        "ha_ema_volume_ema2_period": _coerce_int(raw.get("ha_ema_volume_ema2_period"), DEFAULT_HA_EMA_VOLUME_EMA2_PERIOD, minimum=2, maximum=500),
        "ha_ema_volume_ema2_source": str(raw.get("ha_ema_volume_ema2_source") or DEFAULT_HA_EMA_VOLUME_EMA2_SOURCE).strip().lower(),
        "ha_ema_volume_start_field": str(raw.get("ha_ema_volume_start_field") or DEFAULT_HA_EMA_VOLUME_START_FIELD).strip().lower(),
        "ha_ema_volume_start_line": str(raw.get("ha_ema_volume_start_line") or DEFAULT_HA_EMA_VOLUME_START_LINE).strip().lower(),
        "ha_ema_volume_start_relation": str(raw.get("ha_ema_volume_start_relation") or DEFAULT_HA_EMA_VOLUME_START_RELATION).strip().lower(),
        "ha_ema_volume_end_field": str(raw.get("ha_ema_volume_end_field") or DEFAULT_HA_EMA_VOLUME_END_FIELD).strip().lower(),
        "ha_ema_volume_end_line": str(raw.get("ha_ema_volume_end_line") or DEFAULT_HA_EMA_VOLUME_END_LINE).strip().lower(),
        "ha_ema_volume_end_relation": str(raw.get("ha_ema_volume_end_relation") or DEFAULT_HA_EMA_VOLUME_END_RELATION).strip().lower(),
        "ha_ema_volume_conditions": _normalize_ha_ema_volume_conditions(raw),
        "ha_ema_volume_candle_color": (
            str(raw.get("ha_ema_volume_candle_color") or DEFAULT_HA_EMA_VOLUME_CANDLE_COLOR).strip().lower()
            if str(raw.get("ha_ema_volume_candle_color") or DEFAULT_HA_EMA_VOLUME_CANDLE_COLOR).strip().lower()
            in {"any", "green", "red"}
            else DEFAULT_HA_EMA_VOLUME_CANDLE_COLOR
        ),
        "ha_ema_volume_period": _coerce_int(raw.get("ha_ema_volume_period"), DEFAULT_HA_EMA_VOLUME_PERIOD, minimum=2, maximum=100),
        "ha_ema_volume_multiplier": _coerce_float(raw.get("ha_ema_volume_multiplier"), DEFAULT_HA_EMA_VOLUME_MULTIPLIER, minimum=1.1, maximum=10.0),
        "ha_ema_volume_event_age": _normalize_event_age(raw.get("ha_ema_volume_event_age"), lookback_days=lookback_days, default=DEFAULT_HA_EMA_VOLUME_EVENT_AGE),
        ...
        "ema_slope_period_1": _coerce_int(raw.get("ema_slope_period_1"), 20, minimum=2, maximum=500),
        "ema_slope_period_2": _coerce_int(raw.get("ema_slope_period_2"), 50, minimum=2, maximum=500),
        "ema_slope_period_3": _coerce_int(raw.get("ema_slope_period_3"), 200, minimum=2, maximum=500),
        "ema_slope_1": str(raw.get("ema_slope_1") or raw.get("ema_slope_20") or "any").strip().lower(),
        "ema_slope_2": str(raw.get("ema_slope_2") or raw.get("ema_slope_50") or "any").strip().lower(),
        "ema_slope_3": str(raw.get("ema_slope_3") or raw.get("ema_slope_200") or "any").strip().lower(),
        "timeline_order": [
            key
            for key in (
                raw.get("timeline_order")
                if isinstance(raw.get("timeline_order"), list)
                else DEFAULT_TIMELINE_ORDER
            )
                if str(key) in TIMELINE_RULE_KEYS
                or re.fullmatch(r"(?:rsi|macd|stoch|supertrend|ema_relationship|price_ema|volume_spike|ema_flatten)_[2-3]", str(key))
        ],
    }
    filters["timeline_order"] = list(filters["timeline_order"])
...
def _required_screen_history_days(filters: dict[str, object]) -> int:
    ...
    if bool(filters.get("ema_relationship_enabled")):
        periods.extend([
            int(filters.get("ema_relationship_fast", DEFAULT_EMA_RELATIONSHIP_FAST)),
            int(filters.get("ema_relationship_slow", DEFAULT_EMA_RELATIONSHIP_SLOW)),
        ])
    ...
    if bool(filters.get("ema_flatten_enabled")):
        periods.append(int(filters.get("ema_flatten_period", DEFAULT_EMA_FLATTEN_PERIOD)))
    if bool(filters.get("ha_ema_volume_enabled")):
        periods.extend([
            int(filters.get("ha_ema_volume_ema1_period", DEFAULT_HA_EMA_VOLUME_EMA1_PERIOD)),
            int(filters.get("ha_ema_volume_ema2_period", DEFAULT_HA_EMA_VOLUME_EMA2_PERIOD)),
...
def _get_cached_indicator_history(
    ...
    try:
        with open(os.path.join(INDICATOR_HISTORY_DISK_CACHE, cache_name), "rb") as handle:  # noqa: PTH123
...
def _enrich_indicator_group(group: tuple[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    return scoped[
        [
            "ticker", "date", "high", "low", "close", "volume", "rsi",
...
def _rsi_event_sequence_ages(
    ...
    *,
    ...
    """
    sequence = RsiEventSequence(
        tuple(RsiEvent.from_dict(event) for event in events)
    )
...
def _ema_cross_event_mask(
    ...
    *,
    ...
    normalized_cross_mode = str(cross_mode or DEFAULT_EMA_RELATIONSHIP_CROSS_MODE).strip().lower()
    ...
    fast_matches = pd.Series(True, index=close.index) if fast_direction == "any" else (
        fast_change.gt(0.1) if fast_direction == "positive" else fast_change.lt(-0.1) if fast_direction == "negative" else fast_change.abs().le(0.1)
    )
    slow_matches = pd.Series(True, index=close.index) if slow_direction == "any" else (
        slow_change.gt(0.1) if slow_direction == "positive" else slow_change.lt(-0.1) if slow_direction == "negative" else slow_change.abs().le(0.1)
    )
    fast_slope = "unknown" if fast_change.empty or pd.isna(fast_change.iloc[-1]) else ("positive" if fast_change.iloc[-1] > 0.1 else "negative" if fast_change.iloc[-1] < -0.1 else "flat")
    slow_slope = "unknown" if slow_change.empty or pd.isna(slow_change.iloc[-1]) else ("positive" if slow_change.iloc[-1] > 0.1 else "negative" if slow_change.iloc[-1] < -0.1 else "flat")
...
def _ema_flatten_mask(
    ...
    *,
    ...
    mask = current_flat & (
        states.eq("top") if normalized_mode == "top"
        else states.eq("bottom") if normalized_mode == "bottom"
...
def _volume_spike_mask(volume: pd.Series, *, period: int, multiplier: float) -> tuple[pd.Series, pd.Series]:
...
def _heikin_ashi_ohlc(frame: pd.DataFrame) -> tuple[pd.Series, pd.Series, pd.Series, pd.Series]:
...
def _ha_ema_volume_mask(
    ...
    *,
    ...
    ema1 = pd.to_numeric(ema1_source_series, errors="coerce").ewm(span=ema1_period, adjust=False, min_periods=ema1_period).mean()
    ema2 = pd.to_numeric(ema2_source_series, errors="coerce").ewm(span=ema2_period, adjust=False, min_periods=ema2_period).mean()
    ...
    if conditions is not None:
        ...
        condition_matches = start_value > start_line_value if str(start_relation).lower() == "above" else start_value < start_line_value
        condition_matches &= end_value > end_line_value if str(end_relation).lower() == "above" else end_value < end_line_value
    volume_baseline = volume.ewm(span=volume_period, adjust=False, min_periods=volume_period).mean()
...
def screen_with_controls(
    *,
    ...
    condition_errors = (
        _validate_ha_ema_volume_conditions(normalized_filters["ha_ema_volume_conditions"])
    ...
    for ticker in requested_tickers:
        ...
        try:
            ...
            ema_slope_periods = [int(normalized_filters[f"ema_slope_period_{index}"]) for index in (1, 2, 3)]
            ema_slope_states = {
            ...
            slope_matches = not any(
                normalized_filters[f"ema_slope_{index}"] not in {"any", ema_slope_states[period]}
            ...
            ema_relationship_mask, ema_relationship_states, ema_fast_slope_state, ema_slow_slope_state = _ema_cross_event_mask(
                close,
                fast_period=ema_fast_period,
                slow_period=ema_slow_period,
                fast_slope_direction=str(normalized_filters["ema_relationship_fast_slope"]),
                slow_slope_direction=str(normalized_filters["ema_relationship_slow_slope"]),
            ...
            price_ema_event_results: list[dict[str, object]] = []
            for event in normalized_filters.get("price_ema_events", []):
                ...
                event_mask = _price_ema_cross_mask(event_price, period=event_period, mode=event_mode)
                event_age = _latest_event_age_days(event_mask, dates, lookback_days=lookback_days)
                price_ema_event_results.append({
                    **event,
                    "price_ema_age": event_age,
                    "price_ema_state": f"{event_source} above EMA" if event_price.iloc[-1] > event_ema.iloc[-1] else f"{event_source} below EMA",
                })
            primary_price_ema = price_ema_event_results[0] if price_ema_event_results else {
                "price_ema_source": price_ema_source,
                "price_ema_period": price_ema_period,
                "price_ema_cross_mode": price_ema_mode,
                "price_ema_event_age": int(normalized_filters["price_ema_event_age"]),
                "price_ema_age": None,
            ...
            volume_spike_multiplier = float(normalized_filters["volume_spike_multiplier"])
            ...
            ha_ema_volume_ema1_period = int(normalized_filters["ha_ema_volume_ema1_period"])
            ha_ema_volume_ema1_source = str(normalized_filters["ha_ema_volume_ema1_source"])
            ha_ema_volume_ema2_period = int(normalized_filters["ha_ema_volume_ema2_period"])
            ha_ema_volume_ema2_source = str(normalized_filters["ha_ema_volume_ema2_source"])
            ha_ema_volume_start_field = str(normalized_filters["ha_ema_volume_start_field"])
            ha_ema_volume_start_line = str(normalized_filters["ha_ema_volume_start_line"])
            ha_ema_volume_start_relation = str(normalized_filters["ha_ema_volume_start_relation"])
            ...
            ha_ema_volume_end_relation = str(normalized_filters["ha_ema_volume_end_relation"])
            ha_ema_volume_period = int(normalized_filters["ha_ema_volume_period"])
            ha_ema_volume_multiplier = float(normalized_filters["ha_ema_volume_multiplier"])
            ...
            candidate_pool.append(
                {
                    ...
                    **{f"ema_slope_{index}_state": ema_slope_states[period] for index, period in enumerate(ema_slope_periods, start=1)},
                    ...
                    "volume_spike_ratio": round(volume_spike_ratio_current, 2) if volume_spike_ratio_current is not None else None,
                    "ha_ema_volume_days_ago": ha_ema_volume_age,
                    "ha_ema_volume_ratio": round(ha_ema_volume_ratio_current, 2) if ha_ema_volume_ratio_current is not None else None,
            ...
            if ema_flatten_enabled and not _event_matches_target(
                ...
                continue
            if bool(normalized_filters["volume_spike_enabled"]) and not _event_matches_target(
                volume_spike_age,
            ...
            if bool(normalized_filters["volume_spike_enabled"]):
                event_pairs.append((
                    _volume_spike_label(volume_spike_period, volume_spike_multiplier),
            ...
            matches.append(
                {
                    ...
                    "ema_relationship_fast_slope": ema_fast_slope_state,
                    "ema_relationship_slow_slope": ema_slow_slope_state,
                    "ema_relationship_cross_mode": str(normalized_filters["ema_relationship_cross_mode"]),
                    ...
                    "ema_flatten_days_ago": ema_flatten_age,
                    "ema_flatten_state": ema_flatten_state,
                    "volume_spike_enabled": bool(normalized_filters["volume_spike_enabled"]),
                    ...
                    "reasons": (
                        ...
                        + (
                            [f"{str(result['price_ema_source']).title()} {str(result['price_ema_cross_mode']).replace('_', ' ')} EMA {int(result['price_ema_period'])} {int(result['price_ema_age'] or 0)}d ago" for result in price_ema_event_results]
                            if price_ema_enabled
                            else []
                        )
                        + (
