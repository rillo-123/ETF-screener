...
class Snippet:
    ...
    def get_data(self, ticker: str, days: int = 60) -> pd.DataFrame:
        ...
        if self.auto_fetch and (df.empty or len(df) < days * 0.8):  # 80% threshold
            try:
    ...
    def get_all_data(self, days: int = 60) -> dict[str, pd.DataFrame]:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_ticker = {executor.submit(self.get_data, ticker, days): ticker for ticker in tickers}
            for future in tqdm(as_completed(future_to_ticker), total=len(tickers), desc="Loading all tickers", ncols=80):
    ...
    def map_parallel(self, func: Callable[[str, pd.DataFrame], Any], days: int = 60, desc: str = "Processing") -> list[Any]:
        ...
        """
        results = []
        tickers = list(self.db.get_tickers()) # Materialize to list to ensure length is known
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_ticker = {executor.submit(_worker, ticker): ticker for ticker in tickers}
            for future in tqdm(as_completed(future_to_ticker), total=len(tickers), desc=desc, ncols=80):
                try:
                    res = future.result()
                    if res is not None:
                        results.append(res)
    ...
    def find_oversold_in_period(self, days_lookback: int = 30, rsi_threshold: float = 30) -> dict[str, list]:
    ...
    def find_overbought_in_period(self, days_lookback: int = 30, rsi_threshold: float = 70) -> dict[str, list]:
