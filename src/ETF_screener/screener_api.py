...
class Screener:
    ...
    def filter_close(
        ...
        return self._add_condition(
            "close", gt=gt, gte=gte, lt=lt, lte=lte, eq=eq, ne=ne
        )
    ...
    def filter_pullback(
        ...
        """
        return self._add_condition(
            "pullback", gt=gt, gte=gte, lt=lt, lte=lte, eq=eq, ne=ne
        )
    ...
    def filter_volume(
        ...
        """
        return self._add_condition(
            "volume", gt=gt, gte=gte, lt=lt, lte=lte, eq=eq, ne=ne
        )
    ...
    def filter_swing(
        self, min_pullback: float = 2.0, max_ema_distance: float = 5.0
    ) -> "Screener":
        """
            ...
            for _, row in results.iterrows():
                ...
                try:
                    hist_df = self.db.get_ticker_data(
                        ticker, days=90 if st_config["timeframe"] == "1W" else 60
                    )
                ...
                for _, row in results.iterrows():
                    ...
                    if field not in row or pd.isna(row[field]):
                        try:
                            ...
                            st_timeframe = (
                                self._filters["supertrend"]["timeframe"]
                                if self._filters["supertrend"]
                                else "1D"
                            )
                            st_period = (
                                self._filters["supertrend"]["period"]
                                if self._filters["supertrend"]
                                else 10
                            )
                            st_mult = (
                                self._filters["supertrend"]["multiplier"]
                                if self._filters["supertrend"]
                                else 3.0
                            )
